...
- [ ] `fetchRace` assumes `race/{id}` always returns `{success:1, message: {...}}` on a 200. Confirmed live during manual verification that a nonexistent race ID instead returns HTTP 200 with `{success:0, message:"not_found"}` — a string, not an object — so `json.message.Race_Name`/`RACE_MODE_NAME` silently resolve to `undefined` rather than throwing. In the common case this is masked because `race/results/{id}` for the same bad ID correctly 400s, so `Promise.all` in `loadRaceById` still rejects and the user sees the right "couldn't load that race" message — but if an ID ever exists for one endpoint and not the other, `fetchRace` would resolve with `undefined` fields and `loadRaceById` would show the misleading "unsupported format" error instead of "race not found." Consider checking `json.success === 1` in `apiGet`/`fetchRace` and throwing explicitly. Not fixed now because it isn't reproducible with any real ID found during testing — flagging for awareness, not urgency.

...
import { createLimiter, type CreateLimiterOptions, type RateLimiter } from '../../server/rate-limit.js';
import type { LoadedRace, Provider, Snapshot, SourceRace } from '../../src/domain.js';
...
type Archive = {
  ...
  findCatalog(options?: { query?: string; limit?: number }): SourceRace[];
...
type TestSources = {
  load: Mock<(input: unknown) => Promise<LoadedRace>>;
  refresh: Mock<(sourceRace: SourceRace) => Promise<LoadedRace>>;
};
...
function createTestApp(options: CreateAppOptions): Application {
  return createApp({ trustedProxyIps: [], ...options } as unknown as Parameters<typeof createApp>[0]) as Application;
}
...
function testLimiter(options: CreateLimiterOptions = {}): RateLimiter {
...
describe('archive API', () => {
  ...
  beforeEach(() => {
    ...
    sources = {
      load: vi.fn(async (input: unknown): Promise<LoadedRace> => {
        const sourceRaceId = String(input).match(/\d+/)?.[0];
        return loadedRace(sourceRaceId ? { sourceRaceId } : {});
      }),
      refresh: vi.fn(async (sourceRace: SourceRace): Promise<LoadedRace> => loadedRace({ sourceRaceId: sourceRace.sourceRaceId }))
    };
    app = createTestApp({ archive, sources, limiter: testLimiter() });
  });

...
import type { HistoryEntry, LoadedRace, Provider, RaceEntry, Snapshot, SourceRace } from '../../src/domain.js';

const CAPTURED_AT = '2026-07-17T00:00:00.000Z';
type LoadedRaceOptions = {
  provider?: Provider;
  sourceRaceId?: string;
  canonicalUrl?: string;
  raceName?: string;
...
type Archive = {
  saveSnapshot(loaded: LoadedRace, capturedAt: string): Snapshot;
  getCurrentSnapshot(sourceRaceId: string): Snapshot | null;
  findCatalog(options?: { query?: string; limit?: number }): Array<SourceRace & { capturedAt?: string }>;
  findRacers(query: string): Array<{ normalizedName: string; fullName: string; raceCount: number }>;
  findHistory(normalizedName: string): unknown[];
};
type TableNameRow = { name: string };
type CountRow = { count: number };
type ArtifactBlobRow = { artifact_blob: Buffer };

type HistoryEntryFixture = Omit<Partial<HistoryEntry>, 'entry'> & {
  sourceRaceId: string;
  provider: Provider;
  raceName: string;
  eventDate: string | null;
  eventDateProvenance: HistoryEntry['eventDateProvenance'];
  fullName: string;
...
function historyEntries(entries: HistoryEntryFixture[]): HistoryEntry[] {
  return entries.map((entry) => {
    ...
    return {
      providerSourceRaceId: entry.sourceRaceId.replace(`${entry.provider}:`, ''),
      capturedAt: CAPTURED_AT,
      normalizedName: normalizeRacerName(entry.fullName),
      displayedNumber: null,
      brand: null,
      className: null,
      entry: raceEntry,
...
function tableNames(db: Database.Database): string[] {
  return db
    ...
    .all()
    .map((row) => {
      const tableRow = row as TableNameRow;
...
function artifactBlob(db: Database.Database): Buffer {
  const row = db.prepare('SELECT artifact_blob FROM race_snapshots').get() as ArtifactBlobRow;
  return row.artifact_blob;
}

function mustCurrentSnapshot(archive: Archive, sourceRaceId: string): Snapshot {
  const snapshot = archive.getCurrentSnapshot(sourceRaceId);
  if (!snapshot) throw new Error(`Expected current snapshot for ${sourceRaceId}`);
  return snapshot;
}
...
describe('archive repository', () => {
  ...
  beforeEach(() => {
    ...
    archive = createArchive(db) as Archive;
  });
  ...
  it('keeps immutable snapshots and selects the newest save as current', async () => {
    ...
    expect(countRows(db, 'race_snapshots')).toBe(2);
  });
  ...
  it('does not replace the Current Snapshot with an older capture', () => {
    ...
    expect(mustCurrentSnapshot(archive, 'livelaps:79103').capturedAt).toBe(
  ...
  it('round-trips compressed source artifacts', () => {
    ...
    expect(mustCurrentSnapshot(archive, 'mototally:ECEA/Enduro/2026/6/O1').artifact).toEqual({
      mimeType: 'text/html',
      text: artifactText
    });
    expect(artifactBlob(db)).not.toEqual(Buffer.from(artifactText));
  });
  ...
  it('uses provider and source ID together as the unique Source Race identity', () => {
    ...
    expect(countRows(db, 'source_races')).toBe(2);
    expect(archive.findCatalog({ query: '', limit: 10 }).map(({ id }: SourceRace) => id)).toEqual([
      'mototally:79103',
      'livelaps:79103'
    ]);
  });
  ...
  it('does not write when an upstream refresh fails', async () => {
    archive.saveSnapshot(loadedRace(), CAPTURED_AT);
    const refresh = async (load: () => Promise<LoadedRace>): Promise<Snapshot> => archive.saveSnapshot(await load(), '2026-07-17T01:00:00.000Z');

    await expect(refresh(async () => Promise.reject(new Error('upstream unavailable')))).rejects.toThrow(
      'upstream unavailable'
    );

    expect(countRows(db, 'race_snapshots')).toBe(1);
    expect(mustCurrentSnapshot(archive, 'livelaps:79103').capturedAt).toBe(CAPTURED_AT);
  });

  it('rolls back a snapshot when any nested result cannot be persisted', () => {
    const loaded = loadedRace({ sourceRaceId: 'broken' });
    const firstSection = loaded.normalized.allResults[0]?.sections?.[0];
    if (!firstSection) throw new Error('Expected a section to corrupt');
    const corruptSection = firstSection as { avgSpeed: unknown };
    corruptSection.avgSpeed = Symbol('unsupported');

    expect(() => archive.saveSnapshot(loaded, CAPTURED_AT)).toThrow();
    expect(countRows(db, 'source_races')).toBe(0);
    expect(countRows(db, 'race_snapshots')).toBe(0);
  });
...
describe('history transforms', () => {
  ...
  it('builds separate provider races and percentile trends from matching entries', () => {
    const history = buildRacerHistory(historyEntries([
      {
    ...
    expect(history.races.map((race: { provider: Provider }) => race.provider)).toEqual(['livelaps', 'mototally']);

...
import { lineChart, type LineChartOptions } from '../src/charts.js';
import { mustQuery, mustQueryAll } from './dom-helpers.js';

function render(opts: Omit<LineChartOptions, 'ariaLabel'>): HTMLDivElement {
...
function tickNumbers(container: ParentNode): number[] {
  return mustQueryAll<SVGTextElement>(container, 'text.tick-label')
...
describe('lineChart x-axis labels', () => {
  function xTickLabels(container: ParentNode): Array<string | null> {
    return mustQueryAll<SVGTextElement>(container, 'text.tick-label')
...
describe('lineChart axis direction', () => {
  function pointYs(container: ParentNode): number[] {
    return mustQueryAll<SVGCircleElement>(container, 'circle.pt').map((p) => Number(p.getAttribute('cy')));
  }
  ...
  it('plots lower values higher by default (positions)', () => {
    ...
    const [first = NaN, second = NaN] = pointYs(c);
...
describe('lineChart clamped values', () => {
  it('keeps out-of-range percentile points inside the plot area', () => {
    ...
    const pointYs = mustQueryAll<SVGCircleElement>(c, 'circle.pt').map((p) => Number(p.getAttribute('cy')));
...
describe('lineChart result status markers', () => {
  it('marks official DNFs without dropping their plotted value', () => {
    ...
    expect(mustQueryAll<SVGCircleElement>(c, 'circle.pt')).toHaveLength(2);
    expect(mustQuery<SVGCircleElement>(c, 'circle.pt-dnf')).not.toBeNull();
  });
  ...
  it('shows no-result events as muted baseline markers instead of percentile points', () => {
    ...
    expect(mustQueryAll<SVGCircleElement>(c, 'circle.pt')).toHaveLength(1);
    expect(mustQuery<SVGCircleElement>(c, 'circle.pt-no-result')).not.toBeNull();
    expect(mustQuery<SVGCircleElement>(c, 'circle.pt-no-result').getAttribute('cy')).toBe('188');
  });
});
...
describe('lineChart tooltip values', () => {
  it('uses configured tooltip text instead of the plotted numeric value', () => {
    ...
    const svg = mustQuery<SVGSVGElement>(c, 'svg');
    svg.getBoundingClientRect = () => ({ left: 0, top: 0, width: 520, height: 220 }) as DOMRect;
    Object.defineProperty(c, 'clientWidth', { value: 520 });

    mustQuery<SVGRectElement>(c, 'rect.hit').dispatchEvent(new MouseEvent('pointermove', { clientX: 260, clientY: 40 }));

    expect(mustQuery<HTMLElement>(c, '.tooltip').textContent).toContain('47 / 79 · 42 percentile');
  });
});

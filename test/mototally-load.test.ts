...
type FetchMock = Mock<(input: FetchInput, init?: RequestInit) => Promise<Response>>;
...
function mockFetchReturning(html: string): FetchMock {
  return vi.fn(async (): Promise<Response> => responseText(html));
}
...
describe('resolveAndLoadRace (overall link)', () => {
  it('fetches the pasted O-page and normalizes it', async () => {
    const fetchMock = mockFetchReturning(MOTOTALLY_FIXTURE_HTML);
    ...
    expect(fetchMock).toHaveBeenCalledWith(`${PROXY_PREFIX}ECEA/Enduro/Results.aspx/2026/6/O1/CS`);
  });
});
...
function riderRow(place: number, ama: number): string {
  const t1 = `${place}:00`;
...
function resolutionPage(amaList: number[]): string {
  return `
...
describe('resolveAndLoadRace (class link -> overall resolution)', () => {
  it('resolves a pasted C-page up to the largest containing O-page', async () => {
    ...
    const fetchMock: FetchMock = vi.fn(async (url: FetchInput): Promise<Response> => {
      const group = ['O1', 'O2', 'O5', 'C8'].find((g) => inputUrl(url).includes(`/${g}/`));
      if (!group) throw new Error(`Unexpected URL: ${inputUrl(url)}`);
      const page = pages[group];
      if (!page) throw new Error(`Missing page for ${group}`);
    ...
    expect(fetchMock).toHaveBeenCalledWith(`${PROXY_PREFIX}ECEA/Enduro/Results.aspx/2026/6/C8/CS`);
    expect(fetchMock).toHaveBeenCalledWith(`${PROXY_PREFIX}ECEA/Enduro/Results.aspx/2026/6/O1/CS`);
    expect(fetchMock).toHaveBeenCalledWith(`${PROXY_PREFIX}ECEA/Enduro/Results.aspx/2026/6/O2/CS`);
  });
});

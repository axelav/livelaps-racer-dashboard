...
function runMigrations(db: SqliteDatabase): void {
  ...
  const applied = db.prepare<[string], { 1: number }>('SELECT 1 FROM archive_migrations WHERE name = ?');
  const record = db.prepare<[string, string]>(
    'INSERT INTO archive_migrations (name, applied_at) VALUES (?, ?)'
  );
  const apply = db.transaction((name: string, sql: string): void => {
    db.exec(sql);
...
export function openDatabase(filename: string | Buffer): SqliteDatabase {
